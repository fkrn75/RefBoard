...
// 모든 함수는 BoardImage 배열의 z 값만 변경한다. 렌더(PIXI sprite.zIndex)는 모른다.
...
function sortedBackToFront(items: BoardImage[]): BoardImage[] {
  return items
...
function assignContiguousZ(orderedBackToFront: BoardImage[]): void {
 ...
 */
export function bringToFront(items: BoardImage[], ids: Set<string> | string[]): void {
 ...
 */
export function sendToBack(items: BoardImage[], ids: Set<string> | string[]): void {
  const sel = toIdSet(ids)
  if (sel.size === 0) return
  const ordered = sortedBackToFront(items)
 ...
 */
export function bringForward(items: BoardImage[], ids: Set<string> | string[]): void {
  const sel = toIdSet(ids)
  if (sel.size === 0) return
 ...
 */
export function sendBackward(items: BoardImage[], ids: Set<string> | string[]): void {
  const sel = toIdSet(ids)
  if (sel.size === 0) return
  const ordered = sortedBackToFront(items) // index 0 = 맨 뒤, 마지막 = 맨 앞
 ...
 * 시각적 앞뒤 순서는 그대로 유지된다.
 */
export function normalizeZ(items: BoardImage[]): void {
